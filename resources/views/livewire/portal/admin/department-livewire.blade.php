                ...
                <x-tables>
                    <x-slot name="thead">
                        <tr>
                            ...
                            <th class="border border-gray-600 p-2" style="width: 60%;">Nama Jurusan</th>
                            <th class="border border-gray-600 p-2">Opsi</th>
                        </tr>
                    </x-slot>

                    <x-slot name="tbody">
                        @forelse ($dept as $key => $val)
                            <tr>
                                <td class="border border-gray-600 p-2">{{ 1 + $key }}</td>
                                <td class="border border-gray-600 p-2">{{ "$val->long ($val->abbr)" }}</td>
                    ...
                    <x-slot name="links">{{ $dept->links() }}</x-slot>
                </x-tables>
                ...
                <form wire:submit.prevent="save">
                    <div class="px-4 py-5 sm:p-6 grid grid-cols-6 gap-6">
                        ...
                        <div class="col-span-6 sm:col-span-3">
                            <x-jet-label for="long" value="{{ __('Nama Jurusan') }}" />
                            <x-jet-input id="long" type="text" class="mt-1 block w-full" wire:model.defer="long" />
